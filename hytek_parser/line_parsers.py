...
def b1_parser(line: str, file: ParsedHytekFile) -> None:
    ...
    meet_name = extract(line, 3, 45)
    meet_facility = extract(line, 48, 45)
    meet_start_date = datetime.strptime(extract(line, 93, 8), "%m%d%Y").date()
    meet_end_date = datetime.strptime(extract(line, 101, 8), "%m%d%Y").date()
    meet_altitude = int(extract(line, 117, 5))
